...
public class Map extends JPanel {
	...
	private boolean simultaneous = true; //Refine the blocks on the map all at once or one by one and thus influencing the next block
	...
	public void updateMap(int z, boolean placeBridge) {
		...
		//Updates playerXY and paints the player's previous position with playerWasOn
		...
		if (map[playerXY[0][0]][playerXY[0][1]] == WATER && !Jesus) {
			if (placeBridge && player.getPlayerInv() > 0) {
				...
				//Moves the player in the opposite direction if the player tries to go on water and isn't Jesus
				switch (z) {
				case 0:
					z = 2;
					break;
			...
			//Gets the block the player will be on and uses it to paint it back when the player moves away again
		} else {
	...
	//Places a bridge block in the direction the player last tried to go, if the next block in that direction is water
	public void placeBridge(int[][] playerXY) {
		...
		map[playerXY[0][0]][playerXY[0][1]] = PLAYER;
		map[playerXY[1][0]][playerXY[1][1]] = playerXY[0][2]; //ID of the block that was there before the player
	}

	//Paints the player's new position and paints it's previous position back to its original state
	...
	public int adjacentLand(int x, int y, int search) {
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				if ((x + i >= 0) && (y + j >= 0) && (x + i < map.length) && (y + j < map[0].length) && (map[x + i][y + j] == search)) {
					found++;
				}
			}
		}
		...
		if (!won) {
			...
			for (int i = 0; i < map.length; i++) {
				for (int j = 0; j < map[0].length; j++) {
					...
					} else if (map[i][j] == PLAYER) {//Player color, Red and White
						if (!Jesus) {
					...
					} else if (map[i][j] == PLACEDBRIDGE) {//Placed bridge color
			...
			g.drawLine(width * scale / player.getSpawnWidth() - 3, 0, width * scale / player.getSpawnWidth() - 3, height * scale / player.getSpawnHeight());
			g.drawLine(0, height * scale / player.getSpawnHeight(), width * scale / player.getSpawnWidth() - 3, height * scale / player.getSpawnHeight());
			//The goal spawn area
			g.setColor(Color.RED);
			g.drawLine((width - (width / goal.getSpawnWidth())) * scale, (height - (height / goal.getSpawnHeight())) * scale, width * scale, (height - (height / goal.getSpawnHeight())) * scale);
			g.drawLine((width - (width / goal.getSpawnWidth())) * scale, (height - (height / goal.getSpawnHeight())) * scale, (width - (width / goal.getSpawnWidth())) * scale, height * scale);
